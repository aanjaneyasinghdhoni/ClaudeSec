...
That's it. See the [Quickstart](/quickstart) for what happens next, or [stop and remove](/quickstart#stopping) the service anytime with `claudesec stop`.
...
<CardGroup cols={2}>
  ...
  <Card title="183 threat rules" icon="shield-halved">
...
flowchart LR
    ...
    W --> ING["ingestSpan()"]
    ING --> RULES["183 rules + secret scrub"]
