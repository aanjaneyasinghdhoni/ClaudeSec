...
<CardGroup cols={2}>
  ...
  <Card title="~639 threat rules" icon="shield-halved">
    CRITICAL / HIGH / MEDIUM / LOW regex rules covering prompt injection, credential theft, reverse shells, supply-chain attacks, exfiltration, and recon — evaluated on every span. The CRITICAL tier flags active secret exfiltration off the machine.
  </Card>
...
- **Owner-only database** — `spans_internal.db` is created with `0600` permissions.
...
flowchart LR
    ...
    W --> ING["ingestSpan()"]
    ING --> RULES["~639 rules + secret scrub"]
    RULES --> DB[("SQLite · spans_internal.db · 0600")]
    DB --> IO["Socket.io"]
