...
ClaudeSec is built to **keep your data**. There is no aggressive auto-delete: spans, sessions, and alerts stay in the local SQLite database (`spans_internal.db`) until they cross one of two configured limits, or you prune by hand. This page explains exactly how that works so you can reason about capacity.
...
- **Disk and memory.** More spans means a larger `spans_internal.db` and a larger FTS index. SQLite handles this well, but the file grows roughly linearly with span count.
