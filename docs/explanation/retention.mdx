...
|----------|---------|----------------|
| `CLAUDESEC_MAX_SPANS` | `50000` | Soft ceiling — the **oldest out-of-window** spans are reclaimed above this |
| `CLAUDESEC_RETENTION_DAYS` | `183` | Sessions (and their spans and alerts) older than this are removed. Default is ~6 months, the EU AI Act minimum log-retention floor for high-risk AI systems |
...
Independently of retention, ClaudeSec writes a **consistent binary snapshot** of the live database to `~/.claudesec/backups/` — once shortly after the server boots and daily thereafter, using SQLite's online backup API (which is safe to run while the database is open and being written, unlike a plain file copy under WAL). Backups are created `0600` and pruned to the **most recent 7**. They are a drop-in `.db` you can restore by pointing `CLAUDESEC_DB` at one. This is separate from the hourly human-readable JSON auto-export in `exports/`.
