...
- **SSRF protection on agent fetches.** With the hook installed, a Claude Code
  ...
  `CLAUDESEC_ALLOW_LOCAL_FETCH=1` to allow a local dev server. The check matches
...
- **Tamper-evident logs.** The operator audit log and the enforcement
  ...
  edit to an earlier row is detectable. An optional local key signs the chain,
  and `GET /api/audit/verify` recomputes both chains and reports whether they are
